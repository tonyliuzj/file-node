...
DOCKER_IMAGE="${DOCKER_IMAGE:-tonyliuzj/file-node:latest}"

DIRECT_DATABASE_PATH="${DIRECT_DATABASE_PATH:-data/data.db}"
DOCKER_DATABASE_PATH="${DOCKER_DATABASE_PATH:-/app/data/data.db}"
...
prompt_common_settings() {
  local default_host_port
  local selected_host_port

  step "Generating application settings..."

  FILE_NODE_SECRET="$(openssl rand -base64 32)"
  echo "File Node secret generated."

  default_host_port="${HOST_PORT:-3000}"
  read -p "Port to expose the app on (default ${default_host_port}): " selected_host_port
  HOST_PORT="${selected_host_port:-$default_host_port}"
}
...
# File Node environment configuration

# Persistent server secret for sessions and backend credential encryption.
FILE_NODE_SECRET_KEY=$FILE_NODE_SECRET

# Trust the host header behind reverse proxies and custom domains.
AUTH_TRUST_HOST=true

# SQLite database path, relative to the app directory unless absolute.
...
ensure_app_env_defaults() {
  local database_path="${1:-$DIRECT_DATABASE_PATH}"
  local env_file="${INSTALL_DIR}/${APP_ENV_FILE}"

  ensure_env_value "AUTH_TRUST_HOST" "true" "$env_file"
  ensure_env_value "DATABASE_URL" "$database_path" "$env_file"
}

ensure_compose_env_defaults() {
  local compose_env_file="${INSTALL_DIR}/${COMPOSE_ENV_FILE}"

  ensure_env_value "HOST_PORT" "3000" "$compose_env_file"
  ensure_env_value "CONTAINER_PORT" "$CONTAINER_PORT" "$compose_env_file"
  ensure_env_value "DOCKER_IMAGE" "$DOCKER_IMAGE" "$compose_env_file"
  ensure_env_value "APP_UID" "$RUN_UID" "$compose_env_file"
  ensure_env_value "APP_GID" "$RUN_GID" "$compose_env_file"
}
...
update_direct() {
  ...
  ensure_app_env_defaults "$DIRECT_DATABASE_PATH"
...
install_docker_mode() {
  ...
  as_root chown -R "$RUN_UID:$RUN_GID" "${INSTALL_DIR}/data"
...
update_docker_mode() {
  ...
  ensure_app_env_defaults "$DOCKER_DATABASE_PATH"
  ensure_compose_env_defaults
...
uninstall_docker_mode() {
  ...
  if [ -d "$INSTALL_DIR" ]; then
    if command_exists docker && docker compose version >/dev/null 2>&1 && [ -f "${INSTALL_DIR}/${COMPOSE_FILE}" ]; then
